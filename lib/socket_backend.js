...
Socket.setMethod(function _finalizeAclDataPacket(handle, cid, data) {
	...
	// @TODO: aclDataPkt
	if (Blast.DEBUG) {
		Namespace.log('@TODO: aclDataPkt', handle, cid, data);
	}
 ...
 * @param    {Function} callback
 */
Socket.setMethod(function _createLeConnection(address, address_type, callback) {
	...
	that.sendCommand(DEFS.BLE.CREATE_CONN, payload, function createdConnection(err, status) {
		...
		if (err && err.code !== 0x0B) {
			return callback(err);
		}

		// Already connected?
		that.registerDeviceConnection({address: address});
